...
def sample_task_indices(n_to_sample, n_available_tasks, with_replacement):
    ...
    if with_replacement:
        ...
        for _ in range(math.ceil(n_to_sample / n_available_tasks)):
            sequence = np.arange(n_available_tasks)
...
class EnvPoolSampler(TaskSampler):
    ...
    def grow_pool(self, new_size):
        ...
        if new_size <= len(self._envs):
            return
        to_copy = sample_task_indices(new_size - len(self._envs),
                                  len(self._envs),
